...
mooring_overlap <- getOverlap(det_dist0)
mooring_overlap
```

As `Poverlap = 1` for all three moorings datasets, all our detections have temporal overlap with mooring deployments. 
...
summary(moorDat.l$GBRPPS) # what does the GBRPPS moorings dataset look like?
```
...
```{r, eval=FALSE, message=FALSE, warning=FALSE}
plotDat.l <- mooringDownload(moor_site_codes = "GBRPPS", # have GBRMYR, GBRPPS and NRSYON available
...
<br>
```{r, eval=TRUE, message=FALSE, warning=FALSE}
data_with_mooring_sst_all <- extractMoor(trackingData = det_dist0,
                                         file_loc="imos.cache/moor/temperature",
                                         sensorType = "temperature",
                                         timeMaxh = Inf,
                                         distMaxkm = Inf)
