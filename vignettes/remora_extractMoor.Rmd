...
For example, we might want to plot temperature over depth at the GBRMYR mooring, `GBRMYR`, and then add all the bull shark detections over the same time period from the closest IMOS Animal Tracking Facility installation.


```{r, eval=TRUE, message=FALSE, warning=FALSE}
plotDT(moorData=moorDat.l$GBRMYR, # have GBRMYR, GBRPPS and NRSYON available
       moorName="GBRMYR",
...
We can also visualise current `velocity` data with detections overlaid, where moorings have velocity sensors. For example, we can visualise the zonal, North-South (v) velocity (`vcur`) at the the same mooring mooring, and overlay the bull shark detections associated with that mooring.
