...
# Set workspace
library(data.table); library(dplyr); library(sf); library(leaflet); library(RColorBrewer); library(classInt); library(ncdf4)

# ## IDJ: Below should no longer be required if `remora` pkg is loaded & when appropriate ex. data is available in pkg
# 
# ## set file directory folderloc when developing vignette (need to remove/edit before release)
# folderloc <- "/Users/uqrdwye2/Temp/GitHub/IMOStrack/"
# 
## Source the imos variable file and the functions from the IMOStrack folder
# source(paste0(folderloc,"IMOStrack/R/imos_variables.R"))
# source(paste0(folderloc,"IMOStrack/R/mooringTable.R"))
# source(paste0(folderloc,"IMOStrack/R/getDistance.R"))
# source(paste0(folderloc,"IMOStrack/R/getOverlap.R"))
# source(paste0(folderloc,"IMOStrack/R/mooringDownload.R"))
# source(paste0(folderloc,"IMOStrack/R/extractMoor.R"))
# source(paste0(folderloc,"IMOStrack/R/merge.moor.R"))
# source(paste0(folderloc,"IMOStrack/R/plotDT.R"))
...
## IDJ: replaced with call to .rda compressed file (currently using same QC detections as remora_extractEnv.Rmd)
#test_data <- read_csv(paste0(folderloc,"example data/Townsville Reefs_Bull-sharks/QC_detections.csv"))
load("data/TownsvilleReefQC.RData")
...
det_dist0 <- getDistance(trackingData = qc_data, 
                         moorLocations = moorT)
head(det_dist0)
...
In order to identify which moorings do not have temporal coverage during the period when tag detections were obtained, we have provided the `getOverlap()` function. In this function, we simply enter the detection dataset (`det_dist0`) which also contains the nearest moorings data generated using the above `getDistance()` function. What is returned is a table containing the proportion of tag detections that fell within the coverage period of the mooring deployment at which it was assigned: `moor_site_code`, `moor_coverage_start` and `moor_coverage_end`.

<br>

```{r, eval=TRUE, message=FALSE, warning=FALSE}
# Which moorings have overlapping data with detections?
getOverlap(det_dist0)
```

See that the `moor_site_code = GBRHIN` had very poor temporal coverage during our detection window - only ~3% of tag detections assigned to this mooring from our `getDistance()` function fell within the time window that this mooring was deployed. Let's drop this site from the moorings table (`moorT`) and re-run the `getDistance()` function to find a more appropriate mooring station for these detections which has greater temporal coverage.

```{r, eval=TRUE, message=FALSE, warning=FALSE}
moorT_update <- moorT %>% 
  filter(site_code != "GBRHIN")  # remove this site from our moorings table

## rerun getDistance function with just those moorings with coverage
det_dist <- getDistance(trackingData = qc_data, 
                        moorLocations = moorT_update)

# What is the overlap of detections with moorings without the GBRHIN site
mooring_overlap <- getOverlap(det_dist)
mooring_overlap
```

Much better - all our detections now have temporal overlap with mooring deployments. 
...
#icon.station <- makeAwesomeIcon(icon= 'flag', markerColor = 'darkred', iconColor = 'white')

# make palette for no detections at stations
library(viridisLite)
(domain <- range(qs_stat$num_det)) # get domain of numeric data
...
Although this function provides the capacity to download various sensor types (i.e. `temperature`, `velocity`, `salinity` or `oxygen`), each variable can only be accessed one at a time using the `mooringDownload()` function. The netCDF files should then be saved locally within the working directory (e.g. within the folder `example data/Thredds/temperature`). 
...
head(moorDat.l$CH100) # what does this moorings dataset look like from moor_site_code = CH100?
```
...
We can use the moorings data we've extracted to plot a depth-time plot showing three-dimensional variability in the physical parameter of interest over a specified time period, and then overlay animal tracking detections which are close to this mooring.

For example, we might want to plot temperature over depth at the Heron Island mooring, `GBRHIS`, and then add all the bull shark detections over the same time period from the closest IMOS Animal Tracking Facility installation, which is approximately 10km away.

If we set `addDetections=TRUE`, detections appear as solid vertical lines at the date of detection on the x-axis of the depth-time plot. We can ask for detections to be filtered by CAAB code, tag ID, common name or scientific name. If we set `addDetections=FALSE`, only the depth-time plot of physical data is displayed.


```{r, eval=TRUE, message=FALSE, warning=FALSE}
moorData = moorDat.l$GBRHIS
moorName = "GBRHIS"
varName = "temperature"
...
We can also visualise current `velocity` data with detections overlaid, where moorings have velocity sensors. For example, we can visualise the zonal, North-South (v) velocity (`vcur`) at the One Tree Island mooring, `GBROTE`, and overlay the bull shark detections associated with that mooring, which were detected approximately 9km away. 

We need to make sure that the NetCDF file we have stored locally contains the velocity data. To achieve this, we firsty need to download this netCDF file using the `mooringDownload()` function and then save the file to a different folder location, in this case `example data/Thredds/velocity`.

```{r, eval=TRUE, message=FALSE, warning=FALSE}
# Download the moorings data for One Tree Island (GBROTE)
plotDat.l <- mooringDownload(moor_site_codes = "GBROTE",
...
plotDT(moorData=plotDat.l$GBROTE, 
       moorName="GBROTE",
       dateStart="2013-11-01", 
       dateEnd="2014-02-01", 
       varName = "vcur",
