...
pull_env_arbitrary <- function(urls = NULL, request_type = 'single', study_extent, var_name, folder_name = NULL, .cache, .crop, .output_format = "raster", .parallel = TRUE, .ncores = NULL, verbose = TRUE,
  ...
  #if(!var_name %in% c('rs_sst', 'rs_sst_interpolated', 'rs_salinity', 'rs_chl', 'rs_turbidity', 'rs_npp', 'rs_current', 'bathy', 'dist_to_land', 'hs')){
  ...
  #Workin' on this one to move over from IMOS' github to NOAA's etopo5. 
  if(var_name %in% c("bathy", "dist_to_land")){
    ...
    if(var_name %in% "bathy"){
      ...
      message("Trying with bathyUrl: ", bathyUrl)
      out_brick <-
